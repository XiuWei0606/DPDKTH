function bc = getBCPropertiesMultiphase(bc, model, state)
...
function props  = getReservoirProperties(model, state, bc)
    ...
    [h, p, T, s, rho, mob] = model.getProps(state, 'PhaseInternalEnergy'                 , ...
                                                   'pressure'                 , ...
                                                   'Temperature'              , ...
                                                   's'                        , ...
                                                   'Density'                  , ...
    ...
    if model.dynamicHeatTransFluid
        ...
        % Thw = model.operators.Thw_all(faces);
        % Tho = model.operators.Tho_all(faces);
    end
...
function props = getBoundaryProperties(model, propsRes, bc) 
    ...
    % props = computeFlashGeothermal_TwoPhases(model, props);
end
...
function Tbc = getBoundaryTemperature(model, propsRes, bc)
    sample = propsRes.pressure;
    Tbc = model.AutoDiffBackend.convertToAD(bc.T, sample);
    ...
    if any(is_Hflux)
        ...
        dT            = bc.Hflux(is_Hflux)./(ThR(is_Hflux) + ThW(is_Hflux) + ThO(is_Hflux));
