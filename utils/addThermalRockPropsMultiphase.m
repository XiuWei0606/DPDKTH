function rock = addThermalRockPropsMultiphase( rock, varargin )
...
%}
    ...
    Watt = joule/second;
    opt = struct('lambdaR', 2*Watt/(meter*Kelvin)          , ... % Rock thermal conductivity
                 'lambdaF' , [1, 1, 1]*Watt/(meter*Kelvin) , ... % Fluid thermal conductivity
    ...
    if size(opt.lambdaF, 1) < nc
        opt.lambdaF = repmat(opt.lambdaF, nc, 1);
    end
    rock.lambdaF = opt.lambdaF;
