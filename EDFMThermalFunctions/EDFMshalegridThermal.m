function [G,fracplanes]=EDFMshalegridThermal(G,fracplanes,varargin)
...
for k=1:length(fracturelist)
    tol=opt.Tolerance/10; % reset to default tolerance for subsequent fractures
    ...
    while tightentolerance
        ...
        % at this point, we have an array of possible cells which contain indices of
        ...
        % OMO edit to prevent code from failing because fraccellpoints is
        % empty. This is typically because the fracture area is too small
        assert(~isempty(fraccellpoints),'The fraccellpoints is empty, code should never get here');
        ...
        %OMO edit: added unit normal and planepoint to Fgrid.matrix_connection
        Fgrid.matrix_connection.normal=repmat(planenormal,numel(area),1);
        Fgrid.matrix_connection.planepoint=repmat(planepoint,numel(area),1);
...
for j=1:length(fracturelist)
    ...
    % XW
    FracGrid_new.(fieldname).rock.ntg = ones(FracGrid_new.(fieldname).cells.num,1)*fracplanes(i).ntg;
    FracGrid_new.(fieldname).rock.lambdaR = ones(FracGrid_new.(fieldname).cells.num,1)*fracplanes(i).lambdaR;
    FracGrid_new.(fieldname).rock.lambdaF = ones(FracGrid_new.(fieldname).cells.num,1)*fracplanes(i).lambdaF;
    FracGrid_new.(fieldname).rock.rhoR = ones(FracGrid_new.(fieldname).cells.num,1)*fracplanes(i).rhoR;
    FracGrid_new.(fieldname).rock.CpR = ones(FracGrid_new.(fieldname).cells.num,1)*fracplanes(i).CpR;

    %HR-EDIT:% To indentify the fracture cell with value 
    FracGrid_new.(fieldname).rock.isMatrix = ones(FracGrid_new.(fieldname).cells.num,1)*0;
...
G=assembleShaleGlobalGridThermal(Gtemp);
