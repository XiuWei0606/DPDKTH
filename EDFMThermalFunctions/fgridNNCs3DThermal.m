function [G,fracplanes]=fgridNNCs3DThermal(G,fracplanes,cloudlist,fullline,i,j,fracplanei,fracplanej,tol)
...
nnc_Thr=-1*ones(maxallocate,1); % XW
nnc_Thf=-1*ones(maxallocate,nPh); % XW

% i related data.
icellnodes_par=cell(maxallocate,1);
icellperm_par=zeros(maxallocate,1);
icelllambdaR_par=zeros(maxallocate,1);  % XW
icelllambdaF_par=zeros(maxallocate,nPh);  % XW
icellporo_par=zeros(maxallocate,1);  % XW
for i=1:maxallocate
    k=candidatelist(i,1);
...
%     icellnodes=0.5*(icellnodes(1:(Nicellnodes/2),:)+icellnodes(((Nicellnodes/2)+1):Nicellnodes,:));
    ...
    icelllambdaR_par(i)=Gfi.rock.lambdaR(k);  % XW
    icelllambdaF_par(i, :)=Gfi.rock.lambdaF(k, :);  % XW
    icellporo_par(i)=Gfi.rock.poro(k);  % XW
end

% j related data. 
jcellnodes_par=cell(maxallocate,1); 
jcellperm_par=zeros(maxallocate,1);
jcelllambdaR_par=zeros(maxallocate,1);  % XW
jcelllambdaF_par=zeros(maxallocate,nPh);  % XW
jcellporo_par=zeros(maxallocate,1);  % XW
for i=1:maxallocate
    l=candidatelist(i,2);
...
%     jcellnodes=0.5*(jcellnodes(1:(Njcellnodes/2),:)+jcellnodes(((Njcellnodes/2)+1):Njcellnodes,:));
    ...
    jcelllambdaR_par(i)=Gfj.rock.lambdaR(l);  % XW
    jcelllambdaF_par(i, :)=Gfj.rock.lambdaF(l, :);  % XW
    jcellporo_par(i)=Gfj.rock.poro(l);  % XW
end
...
%     end
    ...
    if xornot
        ...
        % XW
        Trans_Thr_i=(1-icellporo_par(m))*icelllambdaR_par(m)*aperture_i*xlength/df(1);
        Trans_Thr_j=(1-jcellporo_par(m))*jcelllambdaR_par(m)*aperture_j*xlength/df(2);
        Trans_Thr_ij=(Trans_Thr_i*Trans_Thr_j)/(Trans_Thr_i+Trans_Thr_j);
        nnc_Thr(m)=Trans_Thr_ij;
        Trans_Thf_i=icellporo_par(m)*icelllambdaF_par(m, :)*aperture_i*xlength/df(1);
        Trans_Thf_j=jcellporo_par(m)*jcelllambdaF_par(m, :)*aperture_j*xlength/df(2);
        Trans_Thf_ij=(Trans_Thf_i.*Trans_Thf_j)./(Trans_Thf_i+Trans_Thf_j);
        nnc_Thf(m, :)=Trans_Thf_ij;
    end
end

nnc_cells=removeexcess(nnc_cells,-1);
nnc_T=removeexcess(nnc_T,-1);
nnc_Thr=removeexcess(nnc_Thr,-1);  % XW
nnc_Thf=removeexcess(nnc_Thf,-1);  % XW
[~,ind]=unique(nnc_cells,'rows');
nnc_cells = nnc_cells(ind,:);
nnc_T = nnc_T(ind,:);
nnc_Thr = nnc_Thr(ind,:);  % XW
nnc_Thf = nnc_Thf(ind,:);  % XW
G.nnc.cells=[G.nnc.cells;nnc_cells];
G.nnc.T=[G.nnc.T;nnc_T];
G.nnc.Thr=[G.nnc.Thr;nnc_Thr];  % XW
G.nnc.Thf=[G.nnc.Thf;nnc_Thf];  % XW
