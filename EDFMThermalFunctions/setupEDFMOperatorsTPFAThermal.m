function s = setupEDFMOperatorsTPFAThermal(G, rock, tol, varargin)
...
opt = struct('deck', [], 'neighbors', [], 'trans', [], 'thr', [], 'thf', [], 'porv', []); % XW
...
if isempty(Thr)
   ...
   transmult=transmultEDFM(G, tol);
   % transmult=1;
   Thr = Thr.*transmult; % multiply with transmissibility multiplier
   s.Thr_all = Thr; % T_all will not contain nnc transmissibilities!!!!!
   Thr = [Thr;G.nnc.Thr]; % modified line!!!!!!!!!!!!!!!!
   Thr = Thr(intInx);
else
   s.Thr_all = Thr;
   Thr = Thr(intInx);
...
if isempty(Thf)
    for phNo = 1 : nPh
        ...
        transmult=transmultEDFM(G, tol);
        % transmult=1;
        Thf = Thf.*transmult; % multiply with transmissibility multiplier
        Thf_all = [Thf_all Thf]; % T_all will not contain nnc transmissibilities!!!!!
    end
    s.Thf_all = Thf_all;
    Thf = [Thf_all; G.nnc.Thf]; % modified line!!!!!!!!!!!!!!!!
    Thf = Thf(intInx, :);
else
    s.Thf_all = Thf;
    Thf = Thf(intInx);
