function [G,fracplanes]=fracturefractureShaleNNCs3DThermal(G,fracplanes,tol,varargin)
...
if ~isfield(G.nnc,'cells')
    ...
    G.nnc.type={};
    G.nnc.area=[];
    G.nnc.normal=[]; %OMO edit: added unit normal
    G.nnc.planepoint=[]; %OMO edit: added a point in the fracture plane
end
...
for i=1:(numfrac-1)
    ...
    for j=(i+1):numfrac
        ...
        [G,fracplanes]=fgridNNCs3DThermal(G,fracplanes,cloudlist,fullline,i,j,k,l,tol);
...
G.nnc.normal=[G.nnc.normal;zeros(numfracfracnncs,3)]; %OMO edit: added unit normal 
G.nnc.planepoint=[G.nnc.planepoint;zeros(numfracfracnncs,3)]; %OMO edit: added point in frac plane 
