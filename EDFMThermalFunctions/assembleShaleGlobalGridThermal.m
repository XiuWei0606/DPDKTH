function G = assembleShaleGlobalGridThermal(Gm)
...
for i = 1:numel(fieldnames(G.FracGrid))
    ...
    % XW
    G.rock.ntg = [G.rock.ntg; Gf.rock.ntg];
    G.rock.lambdaR = [G.rock.lambdaR; Gf.rock.lambdaR];
    G.rock.lambdaF = [G.rock.lambdaF; Gf.rock.lambdaF];
    G.rock.rhoR = [G.rock.rhoR; Gf.rock.rhoR];
    G.rock.CpR = [G.rock.CpR; Gf.rock.CpR];

    %OMO edit
    if isfield(G.rock,'isMatrix') && isfield(Gf.rock,'isMatrix') && isnumeric(Gf.rock.isMatrix)
        G.rock.isMatrix = [G.rock.isMatrix; Gf.rock.isMatrix];%HR Edit(isMatrix combined)
    end
