function G = pMatFracNNCs3DThermal(G,tol, varargin)
...
%     startIdx = numel(G.nnc.cells(:,1))+1; %this will be the index of first projected cell
    ...
    % XW
    p.Thr = zeros(nRows4pNNC,1);
    p.Thf = zeros(nRows4pNNC,size(G.nnc.Thf, 2));
...
%             localIdxNonEqui = 3*(i-1)+selectedDir;
            ...
            % XW
            [p.Type{localIdxEqui}, p.Area(localIdxEqui), p.Trans(localIdxEqui), p.Thr(localIdxEqui), p.Thf(localIdxEqui, :), p.Dir(localIdxEqui)] = ...
                computeNNCprojArea_n_Trans(G,p.MMneighs(localIdxEqui,:),p.MFcells(localIdxEqui,:),mcells(i,:),fracMatAreas(i));
...
%                     faceSelected(3) = 1;
    ...
    % XW
    p.Thr = p.Thr(idx2keep,:);
    G.nnc.Thr = [G.nnc.Thr; p.Thr];
    p.Thf = p.Thf(idx2keep,:);
    G.nnc.Thf = [G.nnc.Thf; p.Thf];
...
function [pType, pArea, pTrans, pThr, pThf, projDir] = computeNNCprojArea_n_Trans(G,pMMneighs,pMFcells,mcells,fracMatArea)
    if (pMFcells(1)~=0)
...
%        K_pMF = harmmean(G.rock.perm(pMFcells));
        ...
        % XW
        lambdaR_pMF = length(pMFcells)/sum(1./((1-G.rock.poro(pMFcells)).*G.rock.lambdaR(pMFcells))); 
        pThr = lambdaR_pMF*pArea/dist_pMF; 
        lambdaF_pMF = length(pMFcells)./sum(1./(G.rock.poro(pMFcells).*G.rock.lambdaF(pMFcells, :))); 
        pThf = lambdaF_pMF.*pArea/dist_pMF;
...
function [p] = prepareFaceCalcs(p,localIdx,G,mcells,fracMatArea,currFaceID,projectionCells)                  
    ...
    % XW
    [p.Type{localIdx}, p.Area(localIdx), p.Trans(localIdx), p.Thr(localIdx), p.Thf(localIdx, :), p.Dir(localIdx)] = ...
            computeNNCprojArea_n_Trans(G,p.MMneighs(localIdx,:),p.MFcells(localIdx,:),mcells,fracMatArea);                 
end
