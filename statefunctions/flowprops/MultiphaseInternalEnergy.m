classdef MultiphaseInternalEnergy < StateFunction
% State function for internal enery in each fluid phase
    ...
    methods
        %-----------------------------------------------------------------%
        function gp = MultiphaseInternalEnergy(model, varargin)
        ...
        function u = evaluateOnDomain(prop,model, state)
            switch model.thermalFormulation
                ...
                case 'temperature'
                    ...
                    [p, T] = model.getProps(state, 'PhasePressures', ...
                                                      'temperature');
                    ...
                    for i = 1:nph
                        ...
                        u{ix} = prop.evaluateFluid(model, ['u', phases(i)], p{ix}, T);
                    end
            end
        end       
    end
