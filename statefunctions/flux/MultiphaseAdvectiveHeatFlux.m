classdef MultiphaseAdvectiveHeatFlux < StateFunction & UpwindProperty
%State function for computing advective heat flux
    ...
    methods
        %-----------------------------------------------------------------%
        function hf = MultiphaseAdvectiveHeatFlux(model, varargin)
            if nargin < 2
            ...
            hf = hf.dependsOn({'PhaseInternalEnergy'}, 'FlowPropertyFunctions');
        ...
        function H = evaluateOnDomain(prop, model, state)
            ...
            for i = 1:nph
                ...
                H{i} = prop.faceUpstream(model, state, flag{i}, u{i}).*vph;
            end
        end 
    end
